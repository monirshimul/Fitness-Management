                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 ">
                            ...
                            <div class="panel panel-primary">
                                ...
                                <div class="panel-body ">
                                    <c:if test="${df.dietFoodId == null}">
                                        <form action="<%=request.getContextPath()%>/saveDietFood" method="post" >
                                        </c:if>
                                        <c:if test="${df.dietFoodId!= null}">
                                            <form action="<%=request.getContextPath()%>/updateDietFood" method="post">
                                            </c:if>  

                                            <div class="form-group">
                                                <label for="dietFoodId">Diet Food Id</label>
                                                <input value="${df.dietFoodId}"  name="dietFoodId" type="text" class="form-control" id="dietFoodId">
                                            </div>
                                            <div class="form-group">
                                                <label for="dietFoodName">Diet Food Name:</label>
                                                <input value="${df.dietFoodName}" name="dietFoodName" type="text" class="form-control" id="dietFoodName">
                                            </div>
                                            <div class="form-group">
                                                <label for="calorie">Calorie:</label>
                                                <input value="${df.calorie}" name="calorie" type="text" class="form-control" id="calorie">
                                            </div>
                                            ...
                                            <c:if test="${df.dietFoodId != null}">
                                                ...
                                                <a href="<%= request.getContextPath()%>/dietfoodpage" class="btn btn-default pull-right" style="background-color: #F75D59; color: #ffffff">New</a>
                                            </c:if>
                        ...
                        <div class="col-md-8">
                            <table class="table table-bordered table-striped table-responsive">
                                ...
                                <tbody>
                                    <c:forEach var="row" items="${dietfoods}">
                                        <tr>
                                            <td>${row.dietFoodId}</td>
                                            <td>${row.dietFoodName}</td>
                                            <td>${row.calorie}</td>

                                            <td><a href="<%=request.getContextPath()%>/editDietFood/${row.dietFoodId}">EDIT</a></td>
                                            <td>
                                                <a onclick="return confirm('Are you want to delete this item?')"
                                                   href="<%=request.getContextPath()%>/deleteDietFood/${row.dietFoodId}">DELETE</a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                </tbody>
